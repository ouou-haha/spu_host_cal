...
from method import partial_softmax
...
def flash_attention(
    ...
    for j in tqdm(
        ...
        # exp(ai - max) sum(), m
        attn_weight, local_sum, local_max = partial_softmax(attn_weight.float())
...
if __name__ == "__main__":
    ...
    Q = 16
    D = 128
    query_t = torch.rand(B, H, Q, D)  # TODO: type?
    key_t = torch.rand(B, H, K, D)
    value_t = torch.rand(B, H, K, D).to(torch.bfloat16)
    output = torch.zeros(B, H, Q, D)

    flash_attention(query_t, key_t, value_t, output)
    print(output)
=======
#!/usr/bin/env python
# -*- coding:utf-8 -*-
...
>>>>>>> 8ebc166a68e6c765e5520ed984297527ddc8c100
